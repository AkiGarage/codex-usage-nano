...
- ドラッグして好きな位置に置ける小さな usage tab
- MacBook の notch でメニューバーアプリが隠れる問題を回避
- 普段は `C 37%` のような小さい表示だけなので邪魔になりにくい
- 詳細パネルは必要なときだけワンクリックで表示
- メニューバー項目を増やさずに Codex 使用量を確認
...
4. `/Applications/CodexUsageNano.app` を開きます。
...
floating tab を右クリック、または Control-click して `Quit Codex Usage Nano` を選びます。
...
- Tiny usage tab that can be dragged anywhere
- Avoids the notched MacBook menu bar visibility problem
- Stays out of the way with a compact `C 37%` display
- Opens details only when needed, with one click
- Checks Codex usage without adding another menu bar item
...
4. Open `/Applications/CodexUsageNano.app`.
...
Right-click or Control-click the floating tab, then choose `Quit Codex Usage Nano`.

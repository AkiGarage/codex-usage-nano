...
ただし、通常のアプリのようにDockアイコンやメニューバー項目を出しません。`LSUIElement` というmacOSの仕組みを使って、accessory app として動きます。

そのため、使い心地としては widget に近いです。

- Finder や `/Applications` では `.app` として見える
- 起動するとDockには出ない
- メニューバーにも項目を増やさない
- 小さな floating tab が画面に出る
- tabをクリックすると詳細パネルが開く
- もう一度クリックすると閉じる

### アイコンはある？

あります。このアプリ専用のアイコンを入れています。

ただし、実行中はDockに出ない accessory app なので、Dock上に常時アイコンは表示されません。Finder、`/Applications`、Launchpadなどで見たときにアプリのアイコンとして表示されます。

### Control Center に入れられる？

現時点では、このアプリをmacOSのControl Centerへそのまま入れる形にはしていません。

理由は、macOSのControl Centerは、任意のAppKitアプリやfloating panelをそのまま入れる場所ではないためです。少なくともこのアプリの目的である「リサイズできる小さな使用量パネル」をControl Center内にそのまま置く、という作りには向いていません。

このアプリでは、Control Centerやメニューバーに頼らず、画面上の好きな場所に置ける floating tab を採用しています。MacBook の notch で隠れないことを優先した設計です。

### iPhone版は？

iPhoneではmacOSのような常時浮遊パネルをそのまま作るより、Home Screen widget、Lock Screen widget、軽いSwiftUIアプリを組み合わせる方が自然です。

設計案は [iPhone Companion UX Plan](docs/ios-companion-ux-plan.md) にまとめています。Mac側のCodexUsageWidgetがCodexBarCLIから取得した使用量を、認証情報を含まない小さなsnapshotとして同期し、iPhone側はWidgetKitで一目確認できる形にする方針です。

現在のiPhone最小版はUIとwidgetの動作確認用で、表示データはdemo snapshotです。`https://chatgpt.com/codex/cloud/settings/analytics#usage` は人間がブラウザでログインして見るusage画面であり、個人用iPhoneアプリが安全に直叩きできる安定した公開JSON APIとしては扱っていません。実データ化は、Mac側で `CodexBarCLI` から取得したusageだけを小さなsnapshotとして同期する形で進めます。
...
iPhone companion の設計メモは [iPhone Companion UX Plan](docs/ios-companion-ux-plan.md) にあります。
...
It feels like a widget because it is an accessory app:

- It appears as a `.app` in Finder and `/Applications`
- It does not show a Dock icon while running
- It does not add a menu bar item
- It shows a small floating tab
- Clicking the tab toggles the detailed panel

### Does it have an icon?

Yes. The app includes its own icon.

Because it runs as an accessory app, the icon is not shown in the Dock while the app is running. You will see the icon in Finder, `/Applications`, Launchpad, and similar places.

### Can it live in macOS Control Center?

Not in the current version.

macOS Control Center is not a place where an arbitrary AppKit floating panel can simply be inserted. This app intentionally avoids both Control Center and the menu bar. The floating tab design is meant to stay visible even on notched MacBook displays.

### What about iPhone?

On iPhone, the natural version is not a permanently floating panel. The better fit is a Home Screen widget, a Lock Screen widget, and a lightweight SwiftUI app for setup and detail.

The proposed design is documented in [iPhone Companion UX Plan](docs/ios-companion-ux-plan.md). The Mac app would publish a tiny sanitized usage snapshot from CodexBarCLI, and the iPhone side would render it through WidgetKit without storing Codex/OpenAI credentials.

The current minimal iPhone build is for validating the UI and widgets, so it uses demo snapshots. `https://chatgpt.com/codex/cloud/settings/analytics#usage` is a browser-authenticated usage page, not something this personal iPhone app treats as a stable public JSON API. The planned live path is to sync a small sanitized snapshot produced from `CodexBarCLI` on the Mac.
...
The iPhone companion design note is in [iPhone Companion UX Plan](docs/ios-companion-ux-plan.md).

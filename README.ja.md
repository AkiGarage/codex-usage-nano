...
## Codex Usage Nano は、Codex の残り使用量をすぐ確認するための小さな macOS アプリです。フローティングタブは内側の二つのカラーバーでセッション／週次の残トークンを表示し、マウスオーバーでセッション残量の数字を表示し、ワンクリックでコンパクトなセッション／週次の詳細パネルを開きます。詳細パネルは移動でき、macOS 標準の端・角の挙動でサイズ変更でき、Dock アイコンやメニューバー項目を増やさずにリセットできます。
...
10. 有効な詳細パネルの端と角に表示される macOS 標準のリサイズカーソル。
11. `CodexBarCLI` が見つからない、または使用量を返せない場合の短いエラー表示。
...
7. 文字、余白、バー、目印が大きさに合わせて変わる、75% サイズのコンパクトな初期詳細パネル。
...
13. 使用量の取得は、インストール済みの `CodexBarCLI` だけを通すローカル完結の設計。
...
1. GitHub Releases から `CodexUsageNano-0.0.4-macos.zip` をダウンロードします。
2. zip を展開します。
...
詳細パネルの背景部分をドラッグすると、好きな場所へ移動できます。移動後のタブとの位置関係は保存され、次にパネルを開いたときにも再利用されます。

詳細パネルの端または角をドラッグすると、サイズ変更できます。リサイズカーソルは有効な macOS 標準の hit band にだけ出るため、カーソル表示と実際のサイズ変更挙動が一致します。文字、余白、バー、目印はパネルの大きさに合わせて変わります。

詳細パネルをダブルクリックすると、コンパクトな初期サイズに戻ります。パネルをタブとのデフォルト位置関係から動かしていない場合は、そのデフォルト位置関係も復元されます。
